 ...
 */
class TouchDelegateComposite(view: View, var checkVisibility: Boolean = true) : TouchDelegate(
		emptyRect,
		view) {
	...
	companion object {
		 ...
		 */
		@Suppress("UNUSED")
		fun addTouchDelegateOn(
				view: View,
				delegate: AbstractTouchDelegate,
		...
		fun addTouchDelegate(delegate: AbstractTouchDelegate): TouchDelegateComposite {
			val composite = setTouchDelegateCompositeOn(delegate.parentView)
			composite.addDelegate(delegate)
			return composite
		}
